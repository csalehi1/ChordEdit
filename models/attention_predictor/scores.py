...
def calc_norm_deltas(
    ...
    """
    if values.ndim < 2:
        raise ValueError(f"Expected {tuple(values.shape)} == (..., N, C)")
    ...
    if isinstance(baseline_idx, torch.Tensor):
        ...
        if tuple(baseline_idx.shape) != tuple(leading):
            raise ValueError(f"Expected {tuple(baseline_idx.shape)} == {tuple(leading)}")
        c = values.shape[-1]
    ...
    if torch.isnan(baseline).any():
        raise ValueError("Expected a finite baseline cell")
...
def _group_norm_deltas(
    ...
    for _, group in df.groupby(SAMPLE_ID_COL, sort=True):
        ...
        if int(base_mask.sum()) != 1:
            raise ValueError(f"Expected exactly one base row, got {int(base_mask.sum())}")
