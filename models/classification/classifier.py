...
===============================
Accepts two strings and predicts two independent values, each from
{0.0, 0.3, 0.6, 0.9, 1.0}, using ordinal threshold encoding.
...
------------------
    ...
    4. Two independent linear heads each emit 4 raw logits (K-1 thresholds
        for K=5 buckets).
...
BUCKETS = torch.tensor([0.0, 0.3, 0.6, 0.9, 1.0])
"""Ordered output values. Index i maps to BUCKETS[i]."""
...
def ordinal_loss(logits: torch.Tensor, target_idx: torch.Tensor) -> torch.Tensor:
    ...
    entries are 1 and the rest are 0:
...
class OrdinalPairClassifier(nn.Module):
    ...
    Given strings A and B, predicts two independent scalar values each from
    {0.0, 0.3, 0.6, 0.9, 1.0} via cumulative-threshold ordinal decoding.
    """
    ...
    def __init__(
        ...
        Arguments:
            ...
            num_thresholds:  K-1 thresholds for K output buckets (default 4 -> 5 buckets).
        ...
        self.head1 = nn.Linear(mlp_inner, num_thresholds)
        self.head2 = nn.Linear(mlp_inner, num_thresholds)
    ...
    def predict(
        ...
        Returns:
            ...
            {0.0, 0.3, 0.6, 0.9, 1.0}.
        """
        buckets = BUCKETS.to(next(self.parameters()).device)
        self.eval()
        with torch.no_grad():
            l1, l2 = self(strings_a, strings_b)
        return buckets[decode_ordinal(l1)], buckets[decode_ordinal(l2)]
