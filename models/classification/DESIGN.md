...
Given a `(source_prompt, target_prompt)` string pair, predict two diffusion timestep parameters — `t_start` and `t_end` — each taking a value from the ordered set `{0.0, 0.3, 0.6, 0.9, 1.0}`. These parameters control where in the diffusion trajectory the edit is applied.
...
The shared-weight structure gives CORAL two properties that independent `Linear` heads with $128$ to $k_i - 1$ do not have:
